...
By default unit tests use MySQL database but PostgreSQL, Oracle and SQL Server databases are supported as well. Set MONDRIAN_DRIVER environment variable to "mysql" (default), "postgresql", "oracle", "mssql" (jTDS), "sqlserver" (Microsoft JDBC), "vertica", "snowflake", or "clickhouse" to specify database driver that should be used.
...
If using ClickHouse then create database user mondrian_test with password mondrian_test, create database mondrian_test and grant full access to this database for mondrian_test user. Specify environment variable CLICKHOUSE_DATABASE_HOST. Download clickhouse4j-*.jar and copy to spec/support/jars.
...
In case of Vertica, Snowflake, and ClickHouse at first create test data in MySQL and export test data into CSV files and then import CSV files into Vertica or Snowflake (because inserting individual records into these analytical databases is very slow):
...
It is recommended to use RVM (http://rvm.beginrescueend.com) to run tests with different JRuby implementations. mondrian-olap is being tested with latest versions of JRuby 9.2 on Java 8 and 11.
