...
By default unit tests use MySQL database but PostgreSQL, Oracle and SQL Server databases are supported as well. Set MONDRIAN_DRIVER environment variable to "mysql" (default), "postgresql", "oracle", "mssql" (jTDS), "sqlserver" (Microsoft JDBC), "vertica", "snowflake", "clickhouse", "mariadb" to specify database driver that should be used.
